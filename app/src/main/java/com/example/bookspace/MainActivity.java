...
public class MainActivity extends AppCompatActivity {
    ...
    private final ActivityResultLauncher<String> requestPermissionLauncher =
            registerForActivityResult(new ActivityResultContracts.RequestPermission(), isGranted -> {
                if (isGranted) {
                    ...
                    Toast.makeText(this, "Bạn cần cấp quyền thông báo để quản lý nhắc nhở", Toast.LENGTH_SHORT).show();
                }
            });
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.btnSetReminder.setOnClickListener(v -> checkNotificationPermission());
    }
    ...
    private void loadImages() {
        ...
        Glide.with(this).load(URL_BOOK_1).transform(new CenterCrop(), roundedCorners).into(binding.imgBook1);
        Glide.with(this).load(URL_BOOK_2).transform(new CenterCrop(), roundedCorners).into(binding.imgBook2);
        Glide.with(this).load(URL_BOOK_3).transform(new CenterCrop(), roundedCorners).into(binding.imgBook3);
        Glide.with(this).load(URL_BOOK_4).transform(new CenterCrop(), roundedCorners).into(binding.imgBook4);
    }
                ...
                chip.setOnClickListener(v -> {
                    for (int j = 0; j < chipCount; j++) {
                        ...
                        if (other instanceof TextView) {
                            other.setBackground(ContextCompat.getDrawable(this, R.drawable.chip_bg));
